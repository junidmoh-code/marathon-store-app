...
import {
  ...
  MAX_ATTACHMENT_BYTES, STALE_CLAIM_MS, parseEnvText, missingEnvKeys, redactMoney,
} from "./intakeCore.mjs";
...
// ─── NO MONEY IN THE FEED, PROVED AGAINST THE REAL SENTENCES ─────────────────
// /card_batch_intake is readable by everyone holding `card_recon`, where
// /card_batches is owner-only, and the whole justification for that difference
// is that the feed carries outcomes and not figures. The refusal reasons it
// stores are written for the person holding the slip, so they quote the
// figures — and stored verbatim they would have put the batch total, the
// purchase/cash/refund breakdown and the variance sentence itself in front of
// every manager.
//
// The corpus below is generated FROM THE REAL VALIDATORS, not typed out here:
// the strings a test invents are the strings a test passes.
describe("redactMoney — the feed carries outcomes, not figures", () => {
  const { validateExtraction, formatCents } = require("../../functions/lib/card-recon.cjs");

  const realRefusals = () => {
    const out = [];
    const base = {
      tid: "0000HP1X", batchNo: "494",
      openedAt: Date.UTC(2026, 7, 26, 16, 50), closedAt: Date.UTC(2026, 7, 27, 16, 50),
      txnCount: 2, purchasesCents: 5035500, cashCents: 0, refundsCents: 4800, totalCents: 5030700,
      confidence: { tid: 1, batchNo: 1, totalCents: 1, openedAt: 1, closedAt: 1, purchasesCents: 1, txnCount: 1 },
      lines: [
        { tsn: 101, amountCents: 5035500, type: "purchase" },
        { tsn: 102, amountCents: -4800, type: "refund" },
      ],
    };
    // arithmetic that does not hold — quotes all four figures
    out.push(validateExtraction({ ...base, totalCents: 5030701 }, { source: "pdf" }).reason);
    // lines that do not sum to the printed total — quotes two
    out.push(validateExtraction({ ...base, lines: [{ tsn: 101, amountCents: 1, type: "purchase" }, { tsn: 102, amountCents: 2, type: "purchase" }] }, { source: "pdf" }).reason);
    // the photo path's warning form of the same thing
    // The photo path's WARNING form of the same disagreement: the slip's own
    // arithmetic holds, but the lines read off it do not sum to its total. This
    // one lands on a RECORDED row, which is the case that matters most — a
    // refusal at least announces itself.
    out.push(...(validateExtraction({ ...base, lines: [{ tsn: 101, amountCents: 5035500, type: "purchase" }, { tsn: 102, amountCents: -4801, type: "refund" }] }, { source: "photo" }).warnings || []));
    // …and the parser's own "this is not an amount" refusal, which quotes the
    // slip's RAW text — often with no currency mark at all.
    const { moneyField } = require("../../functions/lib/card-recon-pdf.cjs");
    const bare = moneyField(['TOTAL 50,307.00', 'TOTAL 5O,307.00'], /\btotals?\b\s*[:.]?\s*(.*)$/i, "TOTAL");
    if (bare.err) out.push(bare.err);
    return out.filter(Boolean);
  };

  it("the real validators really do quote figures — or this test proves nothing", () => {
    const corpus = realRefusals();
    expect(corpus.length).toBeGreaterThan(2);
    expect(corpus.some((r) => /R[\d,]+\.\d{2}/.test(r))).toBe(true);
  });

  it("strikes every amount out of every one of them", () => {
    for (const reason of realRefusals()) {
      const redacted = redactMoney(reason);
      expect(redacted, `an amount survived: ${redacted}`).not.toMatch(/\d[\d,\s]*\.\d{2}/);
      expect(redacted, `a grouped figure survived: ${redacted}`).not.toMatch(/\d{1,3}(?:[,\s]\d{3})+/);
    }
  });

  it("leaves counts, terminal ids, batch numbers and timestamps alone", () => {
    // What makes a refusal actionable is not the money.
    expect(redactMoney("The slip says 12 transactions but 11 lines were read.")).toContain("12 transactions");
    expect(redactMoney("Terminal 9999ZZZZ is not registered under /config/cardTerminals."))
      .toBe("Terminal 9999ZZZZ is not registered under /config/cardTerminals.");
    expect(redactMoney("Batch #494 for this terminal is already captured.")).toContain("#494");
    expect(redactMoney("Opened 2026/08/26 18:50:04")).toContain("2026/08/26 18:50:04");
    expect(redactMoney("TSN 101, 102 are missing")).toContain("101, 102");
  });

  it("redacts on the way INTO the record — reasons and warnings alike", () => {
    const refused = attachmentOutcome({
      filename: "a.pdf",
      capture: { ok: false, reason: "The transaction lines in that PDF add up to R10.00, but the slip's own total is R50,307.00." },
    });
    expect(refused.reason).not.toMatch(/10\.00|50,307/);
    expect(refused.reason).toContain("R⋯");

    const recorded = attachmentOutcome({
      filename: "b.pdf",
      capture: { recorded: true, batchKey: "494", tid: "0000HP1X", linesCaptured: true,
                 warnings: ["3 transactions on this report (R900.00) have no card sale that answers them. This is the variance."] },
    });
    expect(recorded.warnings[0]).not.toMatch(/900\.00/);
  });

  it("strikes an amount the slip printed WITHOUT a currency mark", () => {
    // The parser quotes the slip's own raw text when a figure will not parse:
    //   The TOTAL reads "50,307.00", which is not an amount this understands.
    // No R, so the currency pattern never sees it — and it is still the batch
    // total. This is what makes the bare-number patterns load-bearing rather
    // than belt-and-braces.
    //
    // NO DIGIT AT ALL should survive in a sentence whose only numbers are
    // amounts. Asserting the absence of one particular substring lets a
    // half-struck figure ("50,R⋯") pass as though it had been redacted.
    expect(redactMoney('The TOTAL reads "50,307.00", which is not an amount this understands.'))
      .not.toMatch(/\d/);
    expect(redactMoney("purchases 50 355.00 against a total of 50 307.00")).not.toMatch(/\d/);
  });

  it("redacts BEFORE clipping, because a cut can leave a remnant redaction cannot catch", () => {
    // Narrower than "half an amount", and real: cut "50,307.00" — no currency
    // mark — at the wrong character and "50,30" is left, which is no longer
    // grouped and has no decimals, so no pattern matches it. A fragment of the
    // batch total, stored, in the node every card_recon holder reads. (An
    // R-prefixed amount survives a cut differently: "R50,30" still looks like
    // money and is still struck.)
    //
    // The space before the figure matters: a number glued to letters has no
    // word boundary in front of it and is not a figure anyone printed.
    const reason = `${"x".repeat(393)} 50,307.00`;
    const row = attachmentOutcome({ filename: "c.pdf", capture: { ok: false, reason } });
    expect(row.reason, `left a fragment: ${row.reason.slice(-14)}`).not.toMatch(/\d/);
    // …and the other order really would have leaked, or this pins nothing.
    expect(redactMoney(clip(reason, 400))).toMatch(/\d/);
  });

  it("passes through what is not a string, and is not fooled by an empty one", () => {
