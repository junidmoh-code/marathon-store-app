// ─── APPLY THE EFT POOL'S RULE TO THE LIVE DOCUMENT ──────────────────────────
// /eft_pool is a TOP-LEVEL node written only by the Mac mini's poller through
// the Admin SDK. Top-level and unnamed, it answers to no rule at all — which is
// why the `".write": "false"` matters: it is not a restriction on the writer
// (the Admin SDK bypasses rules), it is the guarantee that nothing else can
// write there. The read grant is owner-only; the rationale lives in eftRules.mjs.
//
...
//
//   node scripts/cardrecon/apply-eft-pool-rules.mjs           # dry run + diff
//   node scripts/cardrecon/apply-eft-pool-rules.mjs --apply   # write + verify + auto-restore
...
import { EFT_RULE_BLOCKS } from "./eftRules.mjs";
...
const BACKUP = join(homedir(), `rules-live-backup-${stamp}-eft-pool.json`);
...
const untouched = [
  ...
  "card_batch_intake", "card_batch_poll_status", "card_batch_intake_seen",
];
