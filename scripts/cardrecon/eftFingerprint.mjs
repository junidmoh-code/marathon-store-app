...
//
// ─────────────────────────────────────────────────────────────────────────────
// THIS FILE HAS A MIRROR: functions/lib/eft-fingerprint.cjs, which the settle
// callable uses (functions/ is CommonJS and cannot import this one). The two
// must agree EXACTLY — a fingerprint computed at ingestion and a fingerprint
// computed at consume time that disagree would let a duplicate through the very
// guard built to catch it. eftFingerprint.test.mjs runs both against the same
// table and fails if either drifts.
// ─────────────────────────────────────────────────────────────────────────────
...
export function normaliseIdentityText(v) {
...
export function paymentDay(bankTs) {
 ...
 */
export function paymentFingerprint({ reader, bankRef, amountCents, reference, payer, bankTs }) {
 ...
 */
export function admissionDecision({ fingerprint, existing, incomingPoolKey }) {
