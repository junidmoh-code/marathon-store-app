...
const MUTATIONS = [
  ...
  { id: "M-LOCK-HELD", guard: "a fresh claim is held", file: CORE,
    from: "  return Number.isFinite(at) && Math.abs(nowMs - at) < REQUEST_LOCK_MS;", to: "  return false;", tests: T },
  { id: "M-FUTURE-CLAIM", guard: "a claim slightly in the future (clock skew) is held", file: CORE,
    from: "  return Number.isFinite(at) && Math.abs(nowMs - at) < REQUEST_LOCK_MS;", to: "  return Number.isFinite(at) && nowMs - at >= 0 && nowMs - at < REQUEST_LOCK_MS;", tests: T },
  ...
  { id: "M-READ-FAIL-KEEPS", guard: "a failed confirmation read keeps the fence", file: STORE,
    from: "      } catch {\n        return note({ ok: false, message: `The request may or may not have been saved", to: "      } catch {\n        landed = null; if (false) return note({ ok: false, message: `The request may or may not have been saved", tests: T },
  { id: "M-SLOT-REFUSE", guard: "a slot that will not clear refuses the request", file: STORE,
    from: "  if (slot && slot.ok === false) return { ok: false, message: `the display slot could not be cleared (${slot.message})` };", to: "", tests: T },
  { id: "M-TAGGED-UNREAD", guard: "an unread tagged hub never falls through to the other hub", file: CORE,
    from: "    if (!d?.ready && hub === tag) return { hub: null, unread: true };", to: "", tests: T },
  { id: "M-OPEN-OUTRANKS", guard: "an open request outranks a newer resolved one", file: CORE,
    from: "    if (best && best.state === \"requested\" && !open) continue;", to: "", tests: T },
  { id: "M-NOSTOCK-LIVE", guard: "'none in any warehouse' clears when stock returns", file: VIEW,
    from: "    && !pickDisplaySourceHub({ product: c.product || { id: c.productId }, hubData }).hub;", to: ";", tests: R },
  { id: "M-TAP-HANDOVER", guard: "the stream, not a stale tap, decides once it knows the shoe", file: VIEW,
    from: "      if (k.slice(0, i) === store && v.orderId && !streamed.has(String(v.orderId))) ids.add(pid);", to: "      if (k.slice(0, i) === store && v.orderId) ids.add(pid);", tests: R },
  { id: "M-HANDOVER-BY-ORDER", guard: "an older resolved request does not stand in for a fresh tap", file: VIEW,
    from: "      if (r.order?.id != null) streamed.add(String(r.order.id));", to: "      streamed.add(String(r.productId === \"p1\" ? \"042\" : r.order?.id));", tests: R },
  { id: "M-OPEN-IDS", guard: "every open request for the wall is named", file: CORE,
    from: "    if (open) openIds.push(String(o.id));", to: "", tests: R },
  // ── stock / source hub ──
