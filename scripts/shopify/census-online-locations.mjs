...
for (let i = 0; i < livePids.length; i += BATCH) {
  ...
  await Promise.all(slice.map(async (pid) => {
    ...
    for (const [loc, cells] of perLoc) {
      if (!cells) continue;
      const only = networkTotals({ [loc]: { [pid]: cells } }, pid, sizes);
      const u = sum(only);
      if (u > 0) { contrib[loc].units += u; contrib[loc].products += 1; }
    }

    const before = networkTotals(tree, pid, sizes);
    const kept = Object.fromEntries(Object.entries(tree).filter(([loc]) => !EXCLUDE.has(loc)));
    const after = networkTotals(kept, pid, sizes);
    rows.push({
      pid,
...
for (const [loc, v] of order) {
  const counted = loc === "in_transit" ? "  (never counted — transit)" : "";
...
console.log(`  ${"TOTAL (counted)".padEnd(16)} ${String(totalUnits - (contrib.in_transit?.units || 0)).padStart(7)} units`);
