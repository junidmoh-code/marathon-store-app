...
const MUTATIONS = [
  ...
  },
  {
    ...
    file: QUEUE,
    from: `    if (SHOP_DESTS.has(DEST_LOC)) mine = mine.filter(isFirstBatchShopLeg);`,
    to: ``,
    tests: QUEUE_TESTS,
  },
  {
  ...
  },
  {
    ...
    file: SERVER,
    from: `    await seedIfAbsent(db, \`stock/\${FIRST_BATCH_HUB}/\${pid}/\${sizeKey}\`, now);
    // COLD-NULL TRAP`,
    to: `    // COLD-NULL TRAP`,
    nodeTests: OFF_SERVER_TESTS,
  },
  {
    ...
    file: SERVER,
    from: `      if (cur.status !== "open" || (num(cur.sentQty) || 0) > 0) return undefined;
      if (cur.firstBatch && cur.firstBatch.hub2Leg) return undefined;`,
    to: `      if (cur.firstBatch && cur.firstBatch.hub2Leg) return undefined;`,
    nodeTests: OFF_SERVER_TESTS,
  },
  {
    ...
    file: SERVER,
    from: `      return { ...cur, status: "cancelled", cancelReason: PATH_OFF_REASON, resolvedAt: now, resolvedBy: "first_batch_path_off",`,
    to: `      return { ...cur, status: "cancelled", resolvedAt: now, resolvedBy: "first_batch_path_off",`,
    nodeTests: OFF_SERVER_TESTS,
  },
  {
