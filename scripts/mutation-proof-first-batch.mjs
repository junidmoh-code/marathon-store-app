...
const MUTATIONS = [
  ...
  },
  {
    ...
    guard: "only a qty-0 seed stamped at/after the request is excluded from presence (client)",
    file: CORE,
    from: `  const laterSeed = (c) => !!c && c.mv === "seed" && !((Number(c.qty) || 0) > 0)\n    && Number.isFinite(sinceMs) && !!c.updatedAt && Date.parse(c.updatedAt) >= sinceMs;`,
    to: `  const laterSeed = (c) => !!c && c.mv === "seed";`,
    tests: CORE_TESTS,
  },
  {
  ...
  {
    id: "M-GUARD-OPEN-REQUEST-FLAG",
    guard: "the open-Hub-2-request query runs only behind the productId-index flag (never a whole-node read)",
    file: SERVER,
    from: `  if (!config || config.refillRequestsProductIdIndex !== true) return [];`,
    to: ``,
    nodeTests: GUARD_SERVER_TESTS,
  },
  {
    id: "M-GUARD-OPEN-REQUEST-SIGNAL",
    guard: "an open Hub 2 request (no lock) is presence when read",
    file: SERVER,
    from: `  return Object.entries(rows).filter(([, r]) => r && r.status === "open" && r.requestingLocation === FIRST_BATCH_HUB).map(([id]) => id);`,
    to: `  return [];`,
    nodeTests: GUARD_SERVER_TESTS,
  },
  {
    id: "M-GUARD-SEED-CLIENT-KEY",
    guard: "the trigger seeds Hub 2 under the client's cell key ('Free Size' → '_'), never the lock key",
    file: SERVER,
    from: `const clientCellKey = (size) => (size == null || size === "" || size === "Free Size") ? "_" : String(size).replace(/[.#$[\\]/\\s]/g, "_");`,
    to: `const clientCellKey = (size) => encodeSizeKey(size);`,
    nodeTests: GUARD_SERVER_TESTS,
  },
  {
  ...
  },
  {
    ...
    guard: "the server judges seeds and locks against the request's own createdAt",
    file: SERVER,
    from: `    const signals = hub2PresenceSignals({ hub2Node, hub2Locks, hub2OpenRequestIds, sinceIso: rr.createdAt, heldLines, pid });`,
    to: `    const signals = hub2PresenceSignals({ hub2Node, hub2Locks, hub2OpenRequestIds, sinceIso: null, heldLines, pid });`,
    nodeTests: GUARD_SERVER_TESTS,
  },
  {
    ...
    file: SERVER,
    from: `    const signals = hub2PresenceSignals({ hub2Node, hub2Locks, hub2OpenRequestIds, sinceIso: rr.createdAt, heldLines, pid });`,
    to: `    const signals = hub2PresenceSignals({ hub2Node, hub2Locks: null, hub2OpenRequestIds, sinceIso: rr.createdAt, heldLines, pid });`,
    nodeTests: GUARD_SERVER_TESTS,
  },
  {
  ...
  },
  {
    ...
    file: SERVER,
    from: `    if (product) await seedIfAbsent(db, \`stock/\${FIRST_BATCH_HUB}/\${pid}/\${clientCellKey(size)}\`, now);`,
    to: ``,
  ...
  },
  {
    ...
    file: SERVER,
    from: `  const seedNeeded = !hub2Cells || hub2Cells[cellKey] == null;`,
    to: `  const seedNeeded = !hub2Cells || hub2Cells[cellKey] === undefined;`,
    nodeTests: SERVER_TESTS,
  },
  {
