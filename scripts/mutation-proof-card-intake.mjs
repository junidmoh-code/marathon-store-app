...
const MUTATIONS = [
  ...
  ["intake", "a thrown capture error vanishes instead of becoming a refusal",
    ...
    '  if (error) return { filename: name, outcome: "unrelated", reason: text(error, 400) };'],
  ...
  // The redaction is what PAYS FOR the wider read grant on /card_batch_intake.
  // If it stops working the rule is wrong, and the rule is live.
  ["intake", "the figures go back into the feed every card_recon holder can read",
    "  for (const re of MONEY_PATTERNS) out = out.replace(re, MONEY_REDACTED);",
    "  /* mutated */"],

  ["intake", "redaction happens after clipping, so a cut leaves half an amount behind",
    "  const text = (v, max) => clip(redactMoney(v), max);",
    "  const text = (v, max) => redactMoney(clip(v, max));"],

  ["intake", "only currency-marked amounts are struck, so a slip's own printing survives",
