...
say(`# 2. deploy hosting FIRST — it starts forward maintenance, which must be live`);
say(`#    before the index is published (an unmaintained index can over-state carriage)`);
say(`firebase deploy --only hosting:marathon-club`);
say(`# 3. build the index — IN A QUIET WINDOW, before 07:00 or after 19:00 SAST.`);
say(`#    It detects concurrent writes and declines rather than publish a number it`);
say(`#    cannot vouch for, so a daytime run is safe but futile.`);
say(`node scripts/backfill-stock-provenance.mjs --execute`);
say(`# 4. deploy the engine`);
say(`firebase deploy --only functions:refillHealthScan`);
say(`# 5. the withdrawal happens BY ITSELF on the next scan (≤15 min). Verify:`);
say(`node scripts/withdraw-plan-provenance.mjs`);
say("```");
say();
say(`Step 5 needs no command. That is the whole design: the engine retracts its own asks as`);
