...
const MUTATIONS = [
  {
    ...
    from: `        candidates = dropStrictPrefixes([g.code,
  ...
  },
  {
    ...
    from: `  { name: "label-serial", re: /^(?!(?:(?:USM|USW|UKM|UKW|US|UK|EU|EUR|FR|JP|JPN|CM|BR|MX|CN|KR|AU|SIZE)\\d+)+(?:WIDE)?$)(?=[A-Z0-9]{6,16}$)(?:[A-Z]+\\d+){2,}[A-Z]{0,8}$/ },`,
    to: ``,
    nodeTests: ["test/multi-token-label.test.cjs"],
  },
  {
  ...
  },
  {
    ...
    file: "functions/labelAlias/labelAlias.js",
    from: `      for (const o of aliasOwners) raw.push({ code: c, productId: o, via: "alias" });`,
    to: ``,
    nodeTests: ["test/multi-token-label.test.cjs"],
  },
  {
  ...
  },
  {
    ...
    file: "src/components/stock/HubCleanup.jsx",
    from: `          if (claimants.length + anyTokUnloaded.length > 1) {`,
    to: `          if (false) {`,
    tests: ["src/components/stock/hubCleanupAnyToken.render.test.jsx"],
  },
  {
  ...
  {
    id: "M17",
    guard: "A size line (US10UK9 / USM8UK8 / US10UK9WIDE) can never pass as a label-serial (client twin)",
    file: "src/utils/styleCode.js",
    from: `re: /^(?!(?:(?:USM|USW|UKM|UKW|US|UK|EU|EUR|FR|JP|JPN|CM|BR|MX|CN|KR|AU|SIZE)\\d+)+(?:WIDE)?$)(?=[A-Z0-9]{6,16}$)(?:[A-Z]+\\d+){2,}[A-Z]{0,8}$/ },`,
  ...
  {
    id: "M22",
    guard: "The both-ways prefix guard drops truncations from EITHER tier (review #354)",
    file: "functions/styleCode/readStyleCodeLabel.js",
    from: `  return codes.filter((c) => c && !codes.some((t) => t && t !== c && t.startsWith(c)));`,
    to: `  return codes.filter(Boolean);`,
    nodeTests: ["test/multi-token-label.test.cjs"],
  },
  {
    id: "M24",
    guard: "A settled tier-2 pick still caches beside a vision outage (review #354)",
    file: "functions/styleCode/readStyleCodeLabel.js",
    from: `  const settled = !tier2Failed && (!errors.length || candidates.length === 1 || preferred !== null);`,
    to: `  const settled = !tier2Failed && (!errors.length || candidates.length === 1);`,
    nodeTests: ["test/multi-token-label.test.cjs"],
  },
  {
    id: "M25",
    guard: "resolveAnyCode files a same-code owner dispute for a human (review #354)",
    file: "functions/labelAlias/labelAlias.js",
    from: `      if (disputed.length > 1) {`,
    to: `      if (false) {`,
    nodeTests: ["test/multi-token-label.test.cjs"],
  },
  {
    id: "M26",
    guard: "The token-overlap fallback rescues pre-widening labels in the code flow (review #354)",
    file: "src/components/stock/HubCleanup.jsx",
    from: `      if (fallbackTokens.length >= 2) {`,
    to: `      if (false) {`,
    tests: ["src/components/stock/hubCleanupAnyToken.render.test.jsx"],
  },
  {
    id: "M27",
    guard: "Same-code sibling owners get the colourway picker, never merge framing (review #354)",
    file: "src/components/stock/HubCleanup.jsx",
    from: `                siblingSet = allRegisteredSiblings(sharedClaim, claimants.map((c) => c.id));`,
    to: ``,
    tests: ["src/components/stock/hubCleanupAnyToken.render.test.jsx"],
  },
  {
    id: "M28",
    guard: "A failed fetch of a server-proven owner errors — never the link panel (review #354)",
    file: "src/components/stock/HubCleanup.jsx",
    from: `              flash("err", \`Another number on this label matches a registered product, but it couldn't be loaded (\${err?.message || err}) — try again.\`);
              return;`,
    to: `              void err;`,
    tests: ["src/components/stock/hubCleanupAnyToken.render.test.jsx"],
  },
  {
    id: "M29",
    guard: "The gate blocks on ALIAS-ONLY owners via the server round trip (review #354)",
    file: "src/components/admin/StyleCodeGate.jsx",
    from: `          list.unshift({`,
    to: `          if (false) list.unshift({`,
    tests: ["src/components/admin/StyleCodeGate.multiToken.test.jsx"],
  },
  {
    id: "M31",
  ...
  },
  {
    ...
    file: "src/App.jsx",
    from: `        try { anyTok = await resolveAnyCodes(tillAlternates); } catch { /* ranked list below */ }`,
    to: `        void tillAlternates;`,
    tests: ["src/assistantFinderSuggest.pins.test.js"],
  },
  {
    id: "M23",
    guard: "The gender-qualified/width size-line exclusion is load-bearing (client twin)",
    file: "src/utils/styleCode.js",
    from: `(?:(?:USM|USW|UKM|UKW|US|UK|EU|EUR|FR|JP|JPN|CM|BR|MX|CN|KR|AU|SIZE)\\d+)+(?:WIDE)?$`,
    to: `(?:(?:US|UK|EU|EUR|FR|JP|JPN|CM|BR|MX|CN|KR|AU|SIZE)\\d+)+$`,
    tests: ["src/utils/multiTokenLabel.test.js"],
  },
  {
    id: "M19",
