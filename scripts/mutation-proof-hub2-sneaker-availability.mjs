...
const MUTATIONS = [
  ...
  },
  {
    ...
    file: GATE,
    from: `    && order?.placedAtHub !== "hub3" && order?.placedAtHub !== "hubC";`,
    to: `    && order?.placedAtHub !== "hubC";`,
  },
  {
    id: "G6",
    guard: "hubC (customer clothing) rows are NOT probed either",
    file: GATE,
    from: `  return CENTRAL_FED_HUBS.includes(hub)
    && order?.placedAtHub !== "hub3" && order?.placedAtHub !== "hubC";`,
    to: `  return CENTRAL_FED_HUBS.includes(hub) && order?.placedAtHub !== "hub3";`,
  },
  {
    id: "G7",
