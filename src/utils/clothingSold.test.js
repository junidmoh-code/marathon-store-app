...
describe("scope split + photos/category", () => {
  ...
  it("per-store daily tab vs merged backlog (Pine excluded)", () => {
    expect(clothingSoldEventsForPeriod({ ...b, cutoff: TODAY, store: "marathon-pe" }).map(g => g.productId)).toEqual(["p1"]);
    expect(clothingSoldEventsForPeriod({ ...b, cutoff: PAST, store: null, stores: CLOTHING_SOLD_STORES }).map(g => g.store).sort()).toEqual(["marathon-pe", "trophy"]);
  });
