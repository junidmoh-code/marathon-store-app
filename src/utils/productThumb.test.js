...
describe("writeProductThumb — the photo matters more than the thumbnail", () => {
  ...
  it("DELETES the thumbnail it could not replace — stale is worse than missing", async () => {
    // photoUpdatedAt is stamped by the caller either way, and that stamp is the
    // mirror's content marker. A thumbnail of the PREVIOUS photo, recorded as
    // current, is never revisited by any till. A missing one is retried every
    // 6h and regenerated by the POS repo's bulk script.
    const remove = vi.fn().mockResolvedValue(undefined);
    const res = await writeProductThumb("p1", { size: 1 }, {
      upload: async () => { throw new Error("storage/retry-limit-exceeded"); },
      remove, encode: async () => webpBlob(), warn: () => {}, pause: async () => {},
    });
    expect(res.ok).toBe(false);
    expect(res.staleRemoved).toBe(true);
    expect(remove).toHaveBeenCalledWith("products/p1/thumb_300.webp");
  });

  it("removes the stale thumbnail when the ENCODE is what failed", async () => {
    // The Safari-PNG refusal reaches here too, and it is the case most likely
    // to repeat on the same device — all the more reason not to leave a
    // picture of the replaced photo standing.
    const remove = vi.fn().mockResolvedValue(undefined);
    const res = await writeProductThumb("p1", { size: 1 }, {
      upload: vi.fn(), remove,
      encode: async () => { throw new Error("browser encoded image/png, not WebP"); },
      warn: () => {}, pause: async () => {},
    });
    expect(res.ok).toBe(false);
    expect(remove).toHaveBeenCalledWith("products/p1/thumb_300.webp");
  ...
  it("does NOT delete anything when the write succeeds", async () => {
    const remove = vi.fn();
    const res = await writeProductThumb("p1", { size: 1 }, {
      upload: async () => {}, remove, encode: async () => webpBlob(), warn: () => {},
    });
    expect(res.ok).toBe(true);
    expect(remove).not.toHaveBeenCalled();
  });

  it("a failed delete is swallowed too — it never throws and never blocks the save", async () => {
    // The ordinary case is a brand-new product with no thumbnail to remove:
    // storage/object-not-found, and nothing to repair.
    const res = await writeProductThumb("p1", { size: 1 }, {
      upload: async () => { throw new Error("network dropped"); },
      remove: async () => { throw new Error("storage/object-not-found"); },
      encode: async () => webpBlob(), warn: () => {}, pause: async () => {},
    });
    expect(res.ok).toBe(false);
    expect(res.staleRemoved).toBe(false);
  });
