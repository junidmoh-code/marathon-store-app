...
describe("refusalTxn", () => {
  ...
  it("aborts on a request already CLOSED — an engine withdrawal or another person's refusal keeps its shape", () => {
    const withdrawn = { ...OPEN, status: "cancelled", cancelReason: "no_longer_needed", resolvedAt: "2026-09-22T10:00:00.000Z" };
    expect(refusalTxn(withdrawn, FIELDS)).toBeUndefined();
    const refused = { ...OPEN, status: "cancelled", resolvedBy: "u_first", resolvedAt: "2026-09-22T10:00:00.000Z" };
    expect(refusalTxn(refused, FIELDS)).toBeUndefined();
    expect(refusalTxn({ ...OPEN, status: undefined }, FIELDS)).toBeUndefined();
    expect(refusalTxn("junk", FIELDS)).toBeUndefined();
  });
  it("aborts MID-SEND: the tranche's movement is recorded at the sentQty the request is still at", () => {
    expect(refusalTxn(OPEN, FIELDS, { sendingAt: 0 })).toBeUndefined();
  ...
  it("names the tranche movement exactly as Fulfil does", () => {
    ...
    expect(trancheMovementId("abc", undefined)).toBe("rrf_abc");
    expect(trancheMovementId("abc", 2)).toBe("rrf_abc_2");
  });
