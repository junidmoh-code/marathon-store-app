...
describe("PROBE rescan", () => {
  it("selecting the SAME product again still shows its rows", async () => {
    const tree = await renderTab();
    await act(async () => { tree.root.findAllByType("input")[0].props.onChange({ target: { value: "navy" } }); });
    await act(async () => { buttonSaying(tree, "Nike Tee Navy").props.onClick(); });
    await act(async () => {});
    console.log("PROBE first:", text(tree).includes("Trophy"));
    // search and pick the SAME product again
    await act(async () => { tree.root.findAllByType("input")[0].props.onChange({ target: { value: "navy" } }); });
    await act(async () => { buttonSaying(tree, "Nike Tee Navy").props.onClick(); });
    await act(async () => {});
    console.log("PROBE second:", text(tree).includes("Trophy"));
  });
});
