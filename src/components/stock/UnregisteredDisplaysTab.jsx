...
import { unregisteredDisplayCandidates, filterCandidates, brandsOf, openRowsFor, registeredDisplays, rowSizeText } from "./displayRowCore";
...
export default function UnregisteredDisplaysTab({ products = [], orders = [], ordersScope = null, isAdmin = false }) {
  ...
  const closeScanned = async (product, row, reason) => {
    ...
    setNote({ tone: res.warning ? "err" : "ok",
              text: res.warning || `Closed the size ${formatSize(rowSizeText(row))} record at ${labelFor(store)}. No stock moved.` });
  };
              ...
              <div style={{ marginTop: 8 }}>
                {acting === `reg:${g.productId}` ? (
                  <SizePicker
                    ...
                    note={`The record says size ${g.rows.map((r) => formatSize(rowSizeText(r))).join(", ")}. Pick what is there — the old record closes and the new one opens. No stock moves.`}
                    confirmLabel="Register"
