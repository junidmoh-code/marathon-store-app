...
import { computeUnintroduced, migrateToEngine, MIGRATION_DESTS, STANDARD_RUN } from "./introduceExisting";
...
export default function IntroduceExisting({ products = [] }) {
  ...
  const items = useMemo(() => computeUnintroduced(allStock, allTargets, byId), [allStock, allTargets, byId]);
  const migratable = items.filter((i) => i.migratable);
  const numeric = items.filter((i) => !i.migratable);
  const cellEstimate = migratable.reduce((t, i) => t + i.standardSizes.length * MIGRATION_DESTS.length, 0);
    ...
    return (
      <div style={{ ...GLASS, padding: 24, textAlign: "center" }}>
        ...
        <div style={{ color: GRAY, fontSize: 12.5, marginTop: 8, lineHeight: 1.6 }}>
          {result.done} of {result.total} products introduced · {result.cells} target cells written (batch {result.batchId}).<br />
    ...
    <div>
      <div style={{ ...GLASS, padding: "14px 16px", marginBottom: 12 }}>
        ...
        <div style={{ color: GRAY, fontSize: 12, marginTop: 6, lineHeight: 1.65 }}>
          These already circulate at {MIGRATION_DESTS.map((l) => LOC_LABEL[l]).join(" / ")} but were never
          given targets — they are <b>not new products</b> and this is <b>not a decision</b>: your approved standard
          run ({Object.entries(run).map(([s, q]) => `${s}${q}`).join(" ")}) is applied to each product's stocked
          sizes at all three locations ({cellEstimate.toLocaleString()} target cells). From the next scan the engine
          creates the refill and distribution work; the warehouse validates it — nothing moves without a human.
