...
// precomputed by refillHealthScan into ONE small node per store; nothing here
// reads /stock, /products, /stock_movements, /refill_requests or /insights_log.
...
// The only places a snapshot row can name: the two shops, their source (Hub 2)
// and its source (Central). Held here rather than read from /locations so the
// screen keeps its promise literally — it reads the snapshot and nothing else.
// A location outside this set falls through to its raw id, which is honest and
// never blank.
