...
| **`labelModelName`** | **string (≤64)**                 | **no**   | **The model-name line off the label when tier 2 could read one ("DUNK GENESIS"). TIER-2-RESIDUAL ONLY, by cost design: tier 2 fires only when tier 1 could not settle the code, so a label whose code reads cleanly in tier 1 stores no model name — paying a vision call on every clean read for a prefill-only field would invert the funnel. Prefill and ordering only. Omitted when absent.** |
