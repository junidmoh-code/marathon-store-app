...
| `customerId`  | string \| null                                | Canonical `/customers` key (local `0…` phone digits) resolved at order time — the POS loads the customer directly instead of re-resolving by phone. `null` when resolution failed (see `customerPending`), on refill orders, or when the phone had no usable digits (then `customerPending` stays `false` — there is simply no customer to attach). **Added 2026-07** — absent on orders written before. |
