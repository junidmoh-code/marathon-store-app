...
//   "uniform"   { target, minQty, reorderPoint? }
//   "per-size"  { sizes: { "<encodedSize>": { target, minQty, reorderPoint? } } }
//   "invalid"   anything else, INCLUDING both at once
//
// (2026-08-25, superseded same day) A `carriedOnly` carriage-scope flag lived
// in both shapes for a few hours; the owner removed it. A stray `carriedOnly`
// key on a live entry written during that window is IGNORED here (unknown
// keys never decide the entry mode), so a stale node cannot change behaviour.
